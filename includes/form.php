...
class MMTL_Form
{
	...
	static public function number( $args = '' )
	{
		$args = wp_parse_args( $args );
	...
	static public function button( $args = '' )
	{
		extract( wp_parse_args( $args, array
		(
			'id'    => '',
		...
		printf( '<button%s>%s</button>', MMTL_Common::parse_html_attributes(array
		(
			'id'    => $id,
			'class' => $class,
			'name'  => $name,
			'type'  => $type
		), $extra ), esc_html( $value ) );
	}
	...
	static public function options( $options, $selected = '' )
	{
		foreach ( $options as $option_value => $option_text )
		{
			$extra = selected( $option_value, $selected, false );
