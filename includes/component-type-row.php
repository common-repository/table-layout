...
class MMTL_Component_Type_Row
{
	...
	public function add_meta_boxes()
	{
		...
		if ( ! $component_id || $component_id != 'mmtl-row' )
		{
			return;
		}

		add_meta_box( 'mmtl-component-type-row', __( 'Settings', 'table-layout' ), array( $this, 'print_meta_box_inner' ), 'mmtl_component_type' );
	}
	...
	public function print_meta_box_inner( $post )
	{
		...
		$content_max_width = get_post_meta( $post->ID, 'content_max_width', true );
		...
		<h4><?php _e( 'Content', 'table-layout' ); ?></h4>

		<div class="mmtl-form-group">
			<label for="mmtl-content-max-width"><?php _e( 'Max Width', 'table-layout' ); ?></label>
			<div class="mmtl-input-group">
				<input type="number" id="mmtl-content-max-width" class="mmtl-form-control" name="mmtl_content_max_width" value="<?php echo esc_attr( $content_max_width ); ?>">
	...
	public function save_post( $post_id )
	{
		...
		if ( isset( $_POST['mmtl_content_max_width'] ) )
		{
			update_post_meta( $post_id, 'content_max_width', sanitize_text_field( $_POST['mmtl_content_max_width'] ) );
		}
	...
	{
		if ( $component_id == 'mmtl-row' )
		{
			$background_color = get_post_meta( $post_id, 'background_color', true );
			$content_max_width = get_post_meta( $post_id, 'content_max_width', true );
			...
			MMTL_CSS::add_declaration( 'max-width', $content_max_width . 'px', $declarations );

			$css[ $selector . '>.mmtl-content' ] = $declarations;
		}
...
MMTL_Component_Type_Row::get_instance()->init();
