...
class MMTL_Component_Type_Button
{
	...
	public function add_meta_boxes()
	{
		...
		if ( ! $component_id || $component_id != 'mmtl-button' )
		{
			return;
		}

		add_meta_box( 'mmtl-component-type-button', __( 'Settings', 'table-layout' ), array( $this, 'print_meta_box_inner' ), 'mmtl_component_type' );
	}
	...
	public function print_meta_box_inner( $post )
	{
		...
		$border_top_color_hover    = get_post_meta( $post->ID, 'border_top_color_hover', true );
		$border_right_color_hover  = get_post_meta( $post->ID, 'border_right_color_hover', true );
		$border_bottom_color_hover = get_post_meta( $post->ID, 'border_bottom_color_hover', true );
		$border_left_color_hover   = get_post_meta( $post->ID, 'border_left_color_hover', true );
		
		$color_hover = get_post_meta( $post->ID, 'color_hover', true );
		$background_color_hover = get_post_meta( $post->ID, 'background_color_hover', true );
		...
		<p>
			<label for="mmtl-background-color"><?php _e( 'Background color', 'table-layout' ); ?></label><br>
		...
		<h3><?php _e( 'Hover state' ); ?></h3>

		<p><?php _e( 'Border Color', 'table-layout' ); ?></p>

		<?php 

		MMTL_Common::position_fields( 'mmtl_border_%s_color_hover', array
		(
			'top'    => get_post_meta( $post->ID, 'border_top_color_hover', true ),
			'right'  => get_post_meta( $post->ID, 'border_right_color_hover', true ),
			'bottom' => get_post_meta( $post->ID, 'border_bottom_color_hover', true ),
			'left'   => get_post_meta( $post->ID, 'border_left_color_hover', true )
		), 'text', array( 'class' => 'mmtl-color-picker' ) );

		?>

		<p>
			<label for="mmtl-color-hover"><?php _e( 'Color', 'table-layout' ); ?></label><br>
			<input type="text" id="mmtl-color-hover" class="mmtl-color-picker" name="mmtl_color_hover" value="<?php echo esc_attr( $color_hover ); ?>">
		</p>

		<p>
			<label for="mmtl-background-color-hover"><?php _e( 'Background color', 'table-layout' ); ?></label><br>
			<input type="text" id="mmtl-background-color-hover" class="mmtl-color-picker" name="mmtl_background_color_hover" value="<?php echo esc_attr( $background_color_hover ); ?>">
		</p>
	...
	public function save_post( $post_id )
	{
		...
		if ( isset( $_POST['mmtl_border_top_color_hover'] ) )
		{
			update_post_meta( $post_id, 'border_top_color_hover', sanitize_text_field( $_POST['mmtl_border_top_color_hover'] ) );
		}

		if ( isset( $_POST['mmtl_border_right_color_hover'] ) )
		{
			update_post_meta( $post_id, 'border_right_color_hover', sanitize_text_field( $_POST['mmtl_border_right_color_hover'] ) );
		}

		if ( isset( $_POST['mmtl_border_bottom_color_hover'] ) )
		{
			update_post_meta( $post_id, 'border_bottom_color_hover', sanitize_text_field( $_POST['mmtl_border_bottom_color_hover'] ) );
		}

		if ( isset( $_POST['mmtl_border_left_color_hover'] ) )
		{
			update_post_meta( $post_id, 'border_left_color_hover', sanitize_text_field( $_POST['mmtl_border_left_color_hover'] ) );
		}

		if ( isset( $_POST['mmtl_color_hover'] ) )
		{
			update_post_meta( $post_id, 'color_hover', sanitize_text_field( $_POST['mmtl_color_hover'] ) );
		}
		
		if ( isset( $_POST['mmtl_background_color_hover'] ) )
		{
			update_post_meta( $post_id, 'background_color_hover', sanitize_text_field( $_POST['mmtl_background_color_hover'] ) );
		}
	}

	public function css( $css, $post_id, $component_id, $selector )
	{
		if ( $component_id == 'mmtl-button' )
		{
			...
			$border_top_right_radius    = get_post_meta( $post_id, 'border_top_right_radius', true );
			$border_bottom_right_radius = get_post_meta( $post_id, 'border_bottom_right_radius', true );
			$border_bottom_left_radius  = get_post_meta( $post_id, 'border_bottom_left_radius', true );
			$border_top_left_radius     = get_post_meta( $post_id, 'border_top_left_radius', true );
			...
			if ( $alignment = 'left' )
			{
				MMTL_CSS::add_declaration( 'margin-right', 'auto', $declarations );
			}

			else if ( $alignment = 'center' )
			{
				MMTL_CSS::add_declaration( 'margin-left', 'auto', $declarations );
				MMTL_CSS::add_declaration( 'margin-right', 'auto', $declarations );
			}
			...
			$css[ $selector . ':visited' ] = $declarations;

			// hover

			$color_hover = get_post_meta( $post_id, 'color_hover', true );
			$background_color_hover = get_post_meta( $post_id, 'background_color_hover', true );
			$border_top_color_hover    = get_post_meta( $post_id, 'border_top_color_hover', true );
			$border_right_color_hover  = get_post_meta( $post_id, 'border_right_color_hover', true );
			$border_bottom_color_hover = get_post_meta( $post_id, 'border_bottom_color_hover', true );
			$border_left_color_hover   = get_post_meta( $post_id, 'border_left_color_hover', true );

			$declarations = array();

			MMTL_CSS::add_declaration( 'color', $color_hover, $declarations );
			MMTL_CSS::add_declaration( 'background-color', $background_color_hover, $declarations );
			MMTL_CSS::add_declaration( 'border-top-color', $border_top_color_hover, $declarations );
			MMTL_CSS::add_declaration( 'border-right-color', $border_right_color_hover, $declarations );
			MMTL_CSS::add_declaration( 'border-bottom-color', $border_bottom_color_hover, $declarations );
			MMTL_CSS::add_declaration( 'border-left-color', $border_left_color_hover, $declarations );
		
			$css[ $selector . ':active' ] = $declarations;
			$css[ $selector . ':hover' ] = $declarations;
...
MMTL_Component_Type_Button::get_instance()->init();
