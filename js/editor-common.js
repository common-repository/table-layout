...
MMTL_Editor.eventManager.add( 'setup', function( event, ed )
{
	ed.add_filter( 'component_meta', function( meta, shortcode )
	{
		...
		{
			var type = shortcode.get( 'type' );

			if ( type )
			{
				var types = ed.option( 'types' );

				if ( types.hasOwnProperty( type ) )
				{
					var type = types[ type ];

					meta.push( { title : ed.option( 'meta_title_type' ), type : 'type', text : type } );
				};
			};
...
(function()
{
	MMTL_Editor.eventManager.add( 'setup', function( event, ed )
	{
		ed.add_shortcode( 'mmtl-heading', function( attrs, content )
		{
			...
			{
				data.type = ed.components.getType( attrs.type );
			};
